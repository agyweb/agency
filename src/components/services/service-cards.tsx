...
export function SmallHeightScreenServiceCard({
...
export function HorizontallServiceCard({
  service: { description, img, title },
  index,
}: {
  service: CardType;
  ...
  return (
    // <Tilt options={tiltOptions}>
      <motion.div
        ...
        initial="hidden"
        animate={inView ? "visible" : "hidden"}
        className={`relative h-auto w-[280px] shrink-0 rounded-lg bg-gry px-7 py-6 [box-shadow:0_0_0_1px_rgba(0,0,0,.03),0_2px_4px_rgba(0,0,0,.05),0_12px_24px_rgba(0,0,0,.05)] sm:w-80 sm:px-8 lg:w-[350px]`}
      >
        {/* <div className="absolute w-1 left-1/2 h-full bg-red-500 top-0 -translate-x-1/2"></div> */}
        <span className="font-swearDisplay text-xl font-bold sm:text-[22px]">
          0{index}.
